...
dealer::
	@curl -X POST -s \
		...
		| jq '.response.Vehicles|map(select(.Model.Transmission.Desc == "Automatic"))|[.[]|{Price, Color:.Colors.MajorExteriorColor, DealerId, DealerName, Mileage, Model:.Model.Name, Trans:.Model.Transmission.Desc, TrimName:.Model.TrimName, Year:.Model.Year, Vin, Availability, Status, Location: "$(LOCATION)"}]'  >> TMP
